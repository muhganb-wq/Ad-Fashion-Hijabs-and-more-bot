...
def init_db():
    ...
    # Gallery photos beyond the single main photo_file_id. photo_file_id
    # stays as the "main"/thumbnail image for backward compatibility;
    # these are the additional images shown alongside it.
    c.execute("""
...
def get_gallery_photos(product_id):
    """Additional gallery photos beyond the main photo_file_id, in order."""
    c = conn()
    rows = c.execute(
...
def add_gallery_photo(product_id, file_id):
    c = conn()
    next_pos = c.execute(
        "SELECT COALESCE(MAX(position), -1) + 1 AS p FROM product_photos WHERE product_id=?",
        (product_id,),
    ).fetchone()["p"]
    c.execute(
        "INSERT INTO product_photos (product_id, file_id, position) VALUES (?, ?, ?)",
        (product_id, file_id, next_pos),
    )
    c.commit()
    c.close()


def clear_gallery_photos(product_id):
    c = conn()
    c.execute("DELETE FROM product_photos WHERE product_id=?", (product_id,))
    c.commit()
    c.close()


def all_product_photos(product):
    """Main photo (if set) followed by gallery photos, deduplicated, in order."""
    photos = []
    if product["photo_file_id"]:
        photos.append(product["photo_file_id"])
    for file_id in get_gallery_photos(product["id"]):
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lower == "cancel":
        in_admin_flow = bool(
            context.user_data.get("admin_add")
            or context.user_data.get("admin_edit")
            or context.user_data.get("gallery_target")
        )
    ...
    if context.user_data.get("gallery_target"):
        product_id = context.user_data["gallery_target"]
        if lower == "done":
            context.user_data.pop("gallery_target", None)
            count = len(get_gallery_photos(product_id))
            await message.reply_text(
                f"✅ Gallery saved — {count} additional photo{'s' if count != 1 else ''}.",
                reply_markup=admin_back_button(),
            )
        else:
            await message.reply_text(
                "🖼️ Please send a *photo*, or type *done* to finish.",
                parse_mode=ParseMode.MARKDOWN,
            )
        return
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "home":
            ...
            photos = all_product_photos(product)

            if len(photos) >= 2:
                media = [InputMediaPhoto(photos[0], caption=text, parse_mode=ParseMode.MARKDOWN)]
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text="👆 Swipe through the photos above.",
                    reply_markup=keyboard,
                )
                try:
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Admin flow: adding gallery photos for an existing product.
    if is_admin(update) and context.user_data.get("gallery_target"):
        product_id = context.user_data["gallery_target"]
        product = get_product(product_id)
        if not product:
            context.user_data.pop("gallery_target", None)
            await update.message.reply_text(
                "⚠️ That product no longer exists.",
                reply_markup=admin_back_button(),
            )
            return

        file_id = update.message.photo[-1].file_id
        add_gallery_photo(product_id, file_id)
        count = len(get_gallery_photos(product_id))

        await update.message.reply_text(
            f"✅ Added ({count} gallery photo{'s' if count != 1 else ''} so far).\n\n"
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data.startswith("adm:viewp:"):
        ...
        if not product:
            ...
            return
        photo_count = (1 if product["photo_file_id"] else 0) + len(get_gallery_photos(product_id))
        text = (
            f"*{product['name']}*\n"
            f"ID: `{product['id']}`\n"
    ...
    if data.startswith("adm:editp:"):
        ...
        buttons = [
            ...
            [InlineKeyboardButton("🖼️ Gallery Photos", callback_data=f"adm:gallery:{product_id}")],
    ...
    if data.startswith("adm:gallery:"):
        product_id = data.split(":", 2)[2]
        product = get_product(product_id)
        if not product:
            await query.edit_message_text("⚠️ Product not found.", reply_markup=admin_back_button())
        ...
        count = len(get_gallery_photos(product_id))
        await query.edit_message_text(
            f"🖼️ *Gallery Photos — {product['name']}*\n\n"
            f"Additional photos beyond the main one: *{count}*\n\n"
            "Customers see the main photo plus these as a swipeable album.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➕ Add Photos", callback_data=f"adm:addgallery:{product_id}")],
                [InlineKeyboardButton("🗑️ Clear Gallery", callback_data=f"adm:cleargallery:{product_id}")],
                [InlineKeyboardButton("⬅️ Back", callback_data=f"adm:editp:{product_id}")],
            ]),
        )
        return

    if data.startswith("adm:addgallery:"):
        product_id = data.split(":", 2)[2]
        product = get_product(product_id)
        if not product:
            await query.edit_message_text("⚠️ Product not found.", reply_markup=admin_back_button())
            return
        context.user_data["gallery_target"] = product_id
        await query.edit_message_text(
            f"🖼️ Send photos for *{product['name']}* one at a time.\n\n"
            "Type *done* when you're finished, or *cancel* to stop.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    if data.startswith("adm:cleargallery:"):
        product_id = data.split(":", 2)[2]
        product = get_product(product_id)
        if not product:
            await query.edit_message_text("⚠️ Product not found.", reply_markup=admin_back_button())
            return
        await query.edit_message_text(
            f"⚠️ Remove all gallery photos for *{product['name']}*? The main photo is not affected.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("✅ Yes, clear", callback_data=f"adm:cleargalleryconfirm:{product_id}"),
                    InlineKeyboardButton("❌ No", callback_data=f"adm:gallery:{product_id}"),
                ],
            ]),
        )
        return

    if data.startswith("adm:cleargalleryconfirm:"):
        product_id = data.split(":", 2)[2]
        clear_gallery_photos(product_id)
        await query.edit_message_text(
            "✅ Gallery cleared.",
            reply_markup=admin_back_button(),
        )
