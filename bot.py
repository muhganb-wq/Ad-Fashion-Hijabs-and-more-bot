...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        parts = data.split(":")
        if len(parts) != 4:
            await query.edit_message_text("⚠️ Invalid cart selection.", reply_markup=main_menu())
            return
        product_id, color_id_text, qty_text = parts[1], parts[2], parts[3]
        try:
            color_id, qty = int(color_id_text), int(qty_text)
        ...
        product, color = get_product(product_id), get_color_option(color_id)
        if not product or not color or qty < 1:
            await query.edit_message_text("⚠️ Product or fabric/color is no longer available.", reply_markup=main_menu())
            return
        ...
        parts = data.split(":")
        if len(parts) != 4:
            await query.edit_message_text("⚠️ Invalid order selection.", reply_markup=main_menu())
            return
        product_id, color_id_text, qty_text = parts[1], parts[2], parts[3]
        try:
            color_id, qty = int(color_id_text), int(qty_text)
        ...
        parts = data.split(":")
        if len(parts) != 3:
            await query.edit_message_text("⚠️ Invalid quantity selection.", reply_markup=main_menu())
            return
        product_id, color_id_text = parts[1], parts[2]
        try:
            color_id = int(color_id_text)
