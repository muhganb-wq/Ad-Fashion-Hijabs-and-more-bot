...
def cart_keyboard(cart):
    """Cart view keyboard with per-item ➕/➖/🗑️ quantity controls."""
    rows = []
    for key, quantity in cart.items():
        product, _ = cart_product_color(key)
        ...
        rows.append([
            ...
            InlineKeyboardButton(f"{short_name} × {quantity}", callback_data="noop"),
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lower == "cancel":
        in_admin_flow = bool(
            context.user_data.get("admin_add") or context.user_data.get("admin_edit") or context.user_data.get("admin_color_add") or context.user_data.get("admin_edit_welcome") or context.user_data.get("admin_edit_fee")
        )
