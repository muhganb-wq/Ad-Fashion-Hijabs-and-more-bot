...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("shop_qty_target"):
        ...
        subtotal = (product["price"] or 0) * qty
        confirm_text = f"🧾 *Item Selected*\n\nProduct: *{product['name']}*\n🔢 Quantity: *{qty}*\n"
        if product["price"] is not None:
            confirm_text += f"💰 Subtotal: *{money(subtotal)}*\n"
        confirm_text += "\nWhat would you like to do next?"
        await message.reply_text(
            confirm_text,
            parse_mode=ParseMode.MARKDOWN,
        ...
        if data == "home":
            ...
            await safe_edit_query(
                ...
                reply_markup=InlineKeyboardMarkup([
                    ...
                    [InlineKeyboardButton("🔢 Enter Custom Quantity", callback_data=f"plainqtytype:{product_id}")],
                    [InlineKeyboardButton("⬅️ Back to Product", callback_data=f"product:{product_id}")],
                ])
            )
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data == "adm:welcomephoto":
        ...
        text = (
            ...
            "This photo is shown to customers right before the welcome message when they tap /start. "
