...
async def safe_edit_query(query, text, **kwargs):
    ...
    if message is not None and (getattr(message, "photo", None) or getattr(message, "video", None)):
...
def main_menu():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("✨ Send Your Style", callback_data="style")],
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lower == "cancel":
        in_admin_flow = bool(
            context.user_data.get("admin_add") or context.user_data.get("admin_edit") or context.user_data.get("admin_color_add")
        )
    ...
    if context.user_data.get("style_request_step") == "description":
        ...
        c=conn(); cur=c.execute("INSERT INTO style_requests(user_id,username,customer_name,description) VALUES(?,?,?,?)", (user.id, user.username or '', user.full_name or '', description)); request_id=cur.lastrowid; c.commit(); c.close()
        for fid in style.get("photos", []):
            c=conn(); c.execute("INSERT INTO style_request_media(request_id,media_type,file_id) VALUES(?,?,?)", (request_id,"photo",fid)); c.commit(); c.close()
        if style.get("video"):
            c=conn(); c.execute("INSERT INTO style_request_media(request_id,media_type,file_id) VALUES(?,?,?)", (request_id,"video",style["video"])); c.commit(); c.close()
        try:
            await context.bot.send_message(chat_id=ADMIN_ID, text=(f"✨ *NEW STYLE REQUEST #AD-S{request_id:04d}*\n\n👤 Customer: {user.full_name}\nUsername: @{user.username or 'none'}\nTelegram ID: `{user.id}`\n\n📝 Request:\n{description}"), parse_mode=ParseMode.MARKDOWN)
            for fid in style.get("photos", []):
                await context.bot.send_photo(chat_id=ADMIN_ID, photo=fid, caption=f"✨ Style Request #AD-S{request_id:04d} — customer sample")
            if style.get("video"):
                await context.bot.send_video(chat_id=ADMIN_ID, video=style["video"], caption=f"🎥 Style Request #AD-S{request_id:04d} — customer sample video")
        except Exception:
            logger.exception("Could not notify admin about style request")
        context.user_data.pop("style_request", None); context.user_data.pop("style_request_step", None)
        await message.reply_text(f"✅ *Style Request #AD-S{request_id:04d} received.*\n\nOur team has your sample and request. We'll contact you for the next step.", parse_mode=ParseMode.MARKDOWN, reply_markup=main_menu())
        return
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        await safe_edit_query(
            query,
            f"🔢 *Choose quantity for {product['name']}*\n\nYour product photo above is the item you are ordering.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("1", callback_data=f"plainqtyset:{product_id}:1"), InlineKeyboardButton("2", callback_data=f"plainqtyset:{product_id}:2"), InlineKeyboardButton("3", callback_data=f"plainqtyset:{product_id}:3")],
                [InlineKeyboardButton("4", callback_data=f"plainqtyset:{product_id}:4"), InlineKeyboardButton("5", callback_data=f"plainqtyset:{product_id}:5"), InlineKeyboardButton("10", callback_data=f"plainqtyset:{product_id}:10")],
        ...
        selected_text = (
            "✅ *Color/fabric selected.*\n\n"
            f"Product: *{product['name']}*\n"
            f"Selected fabric/color photo: *#{color_id}*\n\n"
            "Now choose how many you want:"
        )
        keyboard = InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("⬅️ Choose Another Color", callback_data=f"colors:{product_id}")],
        ])
        # The color picker button is attached to a PHOTO message. Telegram cannot
        # edit a photo message with edit_message_text; use edit_message_caption.
        ...
        await safe_edit_query(query,
            "✨ *Send Your Style*\n\n"
            "Have a style/sample you want us to recreate or source? Send your photo or video here.\n\n"
            "You can send multiple photos and one video, then tap Done.\n"
            "After that, we'll ask for a short description and send everything to our team.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Done", callback_data="style:done")],[InlineKeyboardButton("❌ Cancel", callback_data="style:cancel")]])
        )
        ...
        if not style or (not style.get("photos") and not style.get("video")):
            await safe_edit_query(query, "Please send at least one sample photo or video first.", reply_markup=main_menu())
            return
        context.user_data["style_request_step"] = "description"
        await safe_edit_query(query, "📝 *One last step*\n\nTell us what you want in a short message.\n\nExample: `I want this style in navy blue.`", parse_mode=ParseMode.MARKDOWN)
        ...
        if row:
            ...
            await safe_edit_query(query, "✅ Thank you. We've notified our team.", reply_markup=main_menu())
        else: await safe_edit_query(query, "Order not found.", reply_markup=main_menu())
...
async def photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    # Customer flow: collect a style/inspiration photo.
    style = context.user_data.get("style_request")
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data.startswith("adm:payverify:"):
        await query.answer("Processing payment verification…")
        parts = data.split(":")
        try:
    ...
    if data == "adm:styles":
        ...
        buttons=[]
        for r in rows:
            lines.append(f"#AD-S{r['id']:04d} — {r['customer_name'] or 'Customer'} — *{r['status']}*\n{r['description'][:160]}")
