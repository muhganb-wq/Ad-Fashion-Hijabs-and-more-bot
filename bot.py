...
def md_escape(text):
    ...
    Critical: any pre-existing backslash in the text must be escaped
    FIRST, before the four special characters. Otherwise a name that
    already contains e.g. "\\_" becomes "\\\\_" — which Telegram reads
    as an escaped backslash followed by an unescaped, entity-opening
    underscore, producing "Can't parse entities: can't find end of the
    entity" for the whole message.
    """
    if text is None:
        return ""
    text = str(text)
...
def get_discount_percent():
    """Site-wide discount on the product subtotal (not delivery fee). 0 = off."""
    try:
        pct = int(get_setting("discount_percent", "0") or "0")
        return max(0, min(100, pct))
    except (ValueError, TypeError):
        return 0


def set_discount_percent(pct):
    set_setting("discount_percent", str(max(0, min(100, int(pct)))))


def discount_amount(items_total):
    """Naira amount knocked off the item subtotal by the active site-wide discount."""
    if items_total is None:
        return 0
    pct = get_discount_percent()
    if pct <= 0:
        return 0
    return round(items_total * pct / 100)


def order_grand_total(items_total, method):
    """Discounted item subtotal plus the delivery fee for the method. Returns
    None if items_total is None (a price-on-request item is in the cart) since
    the true total can't be computed yet — shown separately in that case."""
    if items_total is None:
        return None
    discounted_items = items_total - discount_amount(items_total)
    return discounted_items + get_delivery_fee(method)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    discount_pct = get_discount_percent()
    discount_line = f"\n\n🎉 *{discount_pct}% OFF EVERYTHING — for a limited time!*" if discount_pct else ""
    welcome_body = get_setting("welcome_message", DEFAULT_WELCOME_MESSAGE)
    text=(f"🖤 *{NAME}*\n\n*{TAGLINE}*{vip_line}{discount_line}\n\n" + welcome_body)
...
async def shop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    pct = get_discount_percent()
    banner = f"🎉 *{pct}% OFF EVERYTHING — for a limited time!*\n\n" if pct else ""
    await target.reply_text(
        banner + "🛍️ *Choose a collection:*",
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=main_menu(),
    )
...
async def show_order_confirmation(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price_lines = []
    if discount:
        price_lines.append(f"Subtotal: {money(total)}")
        price_lines.append(f"🎉 {discount_pct}% OFF: -{money(discount)}")
...
async def create_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    price_summary = []
    if discount:
        price_summary.append(f"Subtotal: {money(total)}")
        price_summary.append(f"🎉 {discount_pct}% OFF: -{money(discount)}")
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if lower == "cancel":
        in_admin_flow = bool(
            context.user_data.get("admin_add") or context.user_data.get("admin_edit") or context.user_data.get("admin_color_add") or context.user_data.get("admin_edit_welcome") or context.user_data.get("admin_edit_fee") or context.user_data.get("admin_edit_welcome_photo") or context.user_data.get("admin_edit_discount")
        )
    ...
    if context.user_data.get("admin_edit_discount") and is_admin(update):
        context.user_data.pop("admin_edit_discount", None)
        try:
            pct = int(text.strip().replace("%", ""))
            if pct < 0 or pct > 100:
                raise ValueError
        except ValueError:
            await message.reply_text(
                "⚠️ Please send a whole number between 0 and 100 (e.g. 30).",
                reply_markup=admin_back_button(),
            )
            return
        set_discount_percent(pct)
        if pct:
            await message.reply_text(
                f"✅ Store-wide discount set to {pct}% off everything.",
                reply_markup=admin_back_button(),
            )
        else:
            await message.reply_text("✅ Discount turned off.", reply_markup=admin_back_button())
        return
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data.startswith("adm:editf:"):
        ...
        if field == "subcategory":
            ...
            if valid:
                ...
                hint = f"\n\nValid options for *{md_escape(product['category'])}*:\n{options}\n\nOr type *none* to clear it."
            else:
                hint = f"\n\n_{md_escape(product['category'])} has no subcategories defined — type *none* to clear it._"
        await safe_edit_query(query, 
    ...
    if data == "adm:vieworders":
        ...
        if not rows:
            ...
            lines.append("\nUse /admin\\_orders for full details, or /status ID STATUS to update.")
            text = "\n".join(lines)
    ...
    if data == "adm:settings":
        ...
        text = (
            ...
            f"🎉 Store-wide discount: *{(str(discount_pct) + '%') if discount_pct else 'Off'}*\n\n"
            f"📞 {PHONE1}\n📞 {PHONE2}\n"
            f"📍 {ADDRESS}\n"
            f"🌐 {WEBSITE}\n\n"
            "*Payment accounts:*\n"
        ...
        await safe_edit_query(query, 
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("🎉 Store Discount", callback_data="adm:discount")],
    ...
    if data == "adm:discount":
        pct = get_discount_percent()
        text = (
            "🎉 *Store-Wide Discount*\n\n"
            f"Current: *{(str(pct) + '% off everything') if pct else 'Off'}*\n\n"
            "This applies automatically to every order's item subtotal at checkout "
            "(delivery fees are not discounted). Customers see the discount broken "
            "out on their order confirmation."
        )
        buttons = [[InlineKeyboardButton("✏️ Set Discount %", callback_data="adm:setdiscount")]]
        if pct:
            buttons.append([InlineKeyboardButton("🔴 Turn Off Discount", callback_data="adm:cleardiscount")])
        buttons.append([InlineKeyboardButton("⬅️ Back to Settings", callback_data="adm:settings")])
        await safe_edit_query(query, text, parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup(buttons))
        return

    if data == "adm:setdiscount":
        context.user_data["admin_edit_discount"] = True
        await safe_edit_query(query,
            "🎉 *Set Store Discount*\n\n"
            "Send the discount percentage as a number (e.g. `30` for 30% off), "
            "or type *cancel* to leave it unchanged.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    if data == "adm:cleardiscount":
        set_discount_percent(0)
        await handle_admin_callback(update, context, "adm:discount")
        return
    ...
    if field == "price":
        ...
        if chosen == "none":
            ...
            if valid_keys and chosen not in valid_keys:
                ...
                await message.reply_text(
                    f"⚠️ Not a valid option for *{md_escape(current_category)}*. Choose one:\n\n{options}\n\nOr type *none*.",
                    parse_mode=ParseMode.MARKDOWN,
                )
                return
            value = chosen
