...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        await context.bot.send_message(
            chat_id=ADMIN_ID,
            text=f"⚠️ Bot error: {type(error).__name__}: {error}"[:3500],
        )
    except Exception:
        logger.exception("Could not notify admin about the error.")
    try:
        if isinstance(update, Update) and update.effective_message:
            await update.effective_message.reply_text(
                "⚠️ Something went wrong on our side. Please try again, "
                "or tap /support and we will help you right away."
            )
    except Exception:
        logger.exception("Could not notify the customer about the error.")
...
def md(text):
    """Escape user-supplied text so legacy Markdown messages never fail to send."""
    if text is None:
        return ""
    out = str(text)
    for ch in ("_", "*", "`", "["):
    ...
    if row:
        c.execute("""UPDATE customers SET username=?, full_name=?, last_seen=CURRENT_TIMESTAMP,
                  ...
                  (user.username or '', user.full_name or '',
                   source or '', source or '', referrer_id, user.id))
    else:
...
def record_customer_order(user_id, amount):
    """Keep orders_count, total_spent and the VIP flag accurate after each order."""
    c = conn()
    c.execute("INSERT OR IGNORE INTO customers(user_id) VALUES(?)", (user_id,))
    c.execute("""UPDATE customers
                 SET orders_count = orders_count + 1,
                     total_spent = total_spent + ?,
                     last_seen = CURRENT_TIMESTAMP
                 WHERE user_id=?""", (int(amount or 0), user_id))
    c.execute("""UPDATE customers
                 SET is_vip = 1
                 WHERE user_id=? AND (orders_count >= 3 OR total_spent >= 100000)""",
              (user_id,))
    c.commit(); c.close()
...
async def create_order(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    cust = customer_stats(user.id)
    referrer_id = cust["referrer_id"] if cust else None
    ...
    record_customer_order(user.id, total or 0)
    ...
    admin_text = (
        ...
        f"👤 Customer: {md(customer_name)}\n"
        f"Username: @{md(username) or 'none'}\n"
...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        parts = data.split(":")
        if len(parts) != 3:
            await query.edit_message_text("⚠️ Invalid color selection.", reply_markup=main_menu())
            return
        ...
        if not product or not color:
            await query.edit_message_text("⚠️ That product or color is no longer available.", reply_markup=main_menu())
            return
        ...
        parts = data.split(":")
        if len(parts) != 4:
            await query.edit_message_text("⚠️ Invalid quantity.", reply_markup=main_menu())
            return
        ...
        if not product or not color or qty < 1:
            await query.edit_message_text("⚠️ Product or color is no longer available.", reply_markup=main_menu())
            return
        ...
        await query.edit_message_text(
            f"✅ *Added to your cart.*\n\n{product['name']} × {qty}\n🎨 Selected fabric/color photo: #{color_id}",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 View Cart", callback_data="cart")],[InlineKeyboardButton("🛍️ Continue Shopping", callback_data="shop")],[InlineKeyboardButton("🧾 Checkout", callback_data="checkout")]])
        )
        ...
        colors = get_color_options()
        if colors:
            await query.edit_message_text("Please choose the color/fabric photo first.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎨 Choose Color / Fabric", callback_data=f"colors:{product_id}")]]))
            return
        ...
        if order_now:
            ...
            await query.edit_message_text("✅ *Added to your cart.*", parse_mode=ParseMode.MARKDOWN, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🛒 View Cart", callback_data="cart")],[InlineKeyboardButton("🛍️ Continue Shopping", callback_data="shop")]]))
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data == "adm:colors":
        ...
        text = (
            ...
            "Customers will see these photos and choose the one they want when ordering any product category."
        )
    ...
    if data == "adm:addcolor":
        ...
        await query.edit_message_text(
            ...
            "You do NOT need to type the color name. The photo itself becomes the selectable color/fabric option for customers.\n\n"
            "Type *cancel* to stop.",
            parse_mode=ParseMode.MARKDOWN,
    ...
    if data.startswith("adm:delcolor:"):
        try:
            ...
            await query.edit_message_text("⚠️ Invalid color photo.", reply_markup=admin_back_button())
            return
        color = get_color_option(color_id)
        if not color:
            await query.edit_message_text("⚠️ Color photo not found.", reply_markup=admin_back_button())
            return
        c = conn(); c.execute("UPDATE color_options SET active=0 WHERE id=?", (color_id,)); c.commit(); c.close()
        await query.edit_message_text(f"✅ Fabric/color photo #{color_id} removed from customer choices.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎨 Back to Color/Fabric Photos", callback_data="adm:colors")],[InlineKeyboardButton("🏠 Admin Panel", callback_data="adm:home")]]))
        return
