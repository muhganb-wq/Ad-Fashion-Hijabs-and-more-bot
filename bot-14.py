...
async def buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "home":
        ...
        if not r: await query.answer("Request not found.",show_alert=True); return
        ...
        await query.answer("Delivery selected")
        ...
        await query.answer(); await query.message.reply_text(f"💬 Continue your conversation for #AD-S{rid:04d}. Send your message here and our customer service team will receive it.")
        ...
        if not r or r["quote_status"]!="READY_FOR_CONFIRMATION": await query.answer("Quote is not ready yet.",show_alert=True); return
        c=conn(); c.execute("UPDATE style_requests SET quote_status='CONFIRMED' WHERE id=?",(rid,)); c.commit(); c.close()
        await query.answer("Quote confirmed")
        if not context.user_data.get("style_phone"):
            ...
            await query.message.reply_text("📞 Before payment, please send your phone number for the order.")
        else:
            await style_create_payment_order(update,context,rid)
...
async def handle_admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE, data: str):
    ...
    if data.startswith("adm:stylereply:"):
        rid=int(data.split(":")[2]); context.user_data["style_admin_reply"]=rid; await query.answer(); await query.message.reply_text(f"💬 Type your reply to customer #AD-S{rid:04d} now.") ; return

    if data.startswith("adm:styleprice:"):
        rid=int(data.split(":")[2]); context.user_data["style_admin_price"]=rid; await query.answer(); await query.message.reply_text(f"💰 Enter the agreed PRODUCT price for #AD-S{rid:04d} (numbers only).") ; return
    ...
    if data.startswith("adm:stylequote:"):
        ...
        if not r: await query.answer("Not found",show_alert=True); return
        if not r["delivery_method"]: await query.answer("Customer has not chosen delivery yet.",show_alert=True); return
        context.user_data["style_admin_delivery_fee"]=rid; await query.answer(); await query.message.reply_text(f"🚚 {STYLE_DELIVERY_LABELS.get(r['delivery_method'],r['delivery_method'])}\n\nEnter the DELIVERY FEE for #AD-S{rid:04d} (numbers only).") ; return
